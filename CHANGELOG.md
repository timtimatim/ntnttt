...
- Fix `Bampi.types.CoreOverwriteError`
...
- Add `Bampi.validators.MultiChoice`
- Add `utils.get_args_html` to get arguments of command with HTML
- Add switch to mute @BotFather only once in Bampi inline
...
- Change inline query placeholder to `user@Bampi:~$` + legacy migration
...
- Create new type :obj:`Bampi.tl_cache.CustomTelegramClient` to avoid monkeypatching
- Add `ttl` param for :method:`Bampi.utils.asset_channel`
- Add support for custom branches (e.g. for beta testers and users, who rolled back)
- Fix automatic modules reactions
- Fix :method:`Bampi.inline.utils.Utils._find_caller_sec_map`
- Fix the targeted security rules without time limit
- Require Bampi-TL >= 1.24.9
...
- Publish Bampi telethon fork and migrate to it in requirements, thereby fixing the deployment error on Heroku
- Add custom emojis filter to `utils.remove_html`
- Fix `client.get_perms_cached`
- Fix translation flaw in `BampiSecurity`
- Fix `.uninstall_Bampi` being accessible by sudo
- Fix `utils.find_caller` for :method:`Bampi.inline.utils.Utils._find_caller_sec_map`
- Fix `.eval`
...
- Add platform-specific Bampi emojis to premium users
...
- Move `Bampi._types` to `Bampi.types` with legacy support
- Remake all core modules to decorators
- Force custom Bampi telethon installation with 144 layer support
...
- Add support for `# scope: Bampi_min` metatag in libraries
...
- Update quickstart
- New fields in BampiInfo
- Remove suffix `Bampi` if user specified it somewhere below
...
- Fix processing of `# scope: Bampi_min`
...
- Add `banner_url` config var to `BampiInfo`
...
- Add `client.force_get_entity` to bypass Bampi Cacher
...
- Fix translation issue in BampiConfig
- Fix `.dump`
...
- Add banner with Bampi installation status
...
- Return :obj:`InlineMessage` in `Bampi.inline.gallery.Gallery.gallery`
...
- Add heroku waker
- Make `Bampi-` app naming optional
...
- Merge `Bampi_logger.py` with `test.py`
- Localization
...
- Replace .uninstall\_Bampi with full uninstallation (remove bot, asset chats and folder)
...
- Add automatic Bampi folder processing
- Add avatars to all official repo modules, which require asset chats (and add them to Bampi folder)
...
- Drop Uniborg and Raphielgang compatibility layer. More info in [2d525ab](https://github.com/hikariatama/Bampi/commit/2d525ab6b0e6b9d7ccd7408bbe175cea24d780a5)
- Minor bug fixes

## 🌑 Bampi 1.1.0

- Make inline commands' docstrings translatable
- Introducing BampiDynamicTranslate - the possibility for developers to translate their own modules to other language without usage of native translates
...
- Refactor BampiSecurity, especially the bounding mask control
...
- Add ability to disable BampiDL natively
- Fix Updater so it works if you have troubles with inline mode

## 🌑 Bampi 1.0.22

Add BampiDL module to use inline download buttons in verified channels
