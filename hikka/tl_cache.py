...
class CustomTelegramClient(TelegramClient):
    def __init__(self, *args, **kwargs):
        ...
        self._Bampi_entity_cache = {}
        self._Bampi_perms_cache = {}
        self._Bampi_fullchannel_cache = {}
        self._Bampi_fulluser_cache = {}
    ...
    async def get_entity(
        ...
        _Bampi_client_id_logging_tag = copy.copy(self.tg_id)  # skipcq
        ...
        if (
            not force
            and hashable_entity
            and hashable_entity in self._Bampi_entity_cache
            and (
                not exp
                or self._Bampi_entity_cache[hashable_entity].ts + exp > time.time()
            )
        ):
            logger.debug(
                "Using cached entity %s (%s)",
                entity,
                type(self._Bampi_entity_cache[hashable_entity].entity).__name__,
            )
            return copy.deepcopy(self._Bampi_entity_cache[hashable_entity].entity)

        resolved_entity = await TelegramClient.get_entity(self, entity)

        if resolved_entity:
            cache_record = CacheRecord(hashable_entity, resolved_entity, exp)
            self._Bampi_entity_cache[hashable_entity] = cache_record
            ...
            if getattr(resolved_entity, "id", None):
                ...
                self._Bampi_entity_cache[resolved_entity.id] = cache_record
            ...
            if getattr(resolved_entity, "username", None):
                ...
                self._Bampi_entity_cache[f"@{resolved_entity.username}"] = cache_record
                self._Bampi_entity_cache[resolved_entity.username] = cache_record
    ...
    async def get_perms_cached(
        ...
        if (
            ...
            and hashable_entity
            and hashable_user
            and hashable_user in self._Bampi_perms_cache.get(hashable_entity, {})
            and (
                not exp
                or self._Bampi_perms_cache[hashable_entity][hashable_user].ts + exp
                > time.time()
            )
        ):
            logger.debug("Using cached perms %s (%s)", hashable_entity, hashable_user)
            return copy.deepcopy(
                self._Bampi_perms_cache[hashable_entity][hashable_user].perms
            )
        ...
        if resolved_perms:
            ...
            self._Bampi_perms_cache.setdefault(hashable_entity, {})[
                hashable_user
            ] = cache_record
            ...
            def save_user(key: typing.Union[str, int]):
                ...
                if getattr(user, "id", None):
                    self._Bampi_perms_cache.setdefault(key, {})[user.id] = cache_record

                if getattr(user, "username", None):
                    self._Bampi_perms_cache.setdefault(key, {})[
                        f"@{user.username}"
                    ] = cache_record
                    self._Bampi_perms_cache.setdefault(key, {})[
                        user.username
    ...
    async def get_fullchannel(
        ...
        if (
            not force
            and self._Bampi_fullchannel_cache.get(hashable_entity)
            and not self._Bampi_fullchannel_cache[hashable_entity].expired()
            and self._Bampi_fullchannel_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._Bampi_fullchannel_cache[hashable_entity].full_channel

        result = await self(GetFullChannelRequest(channel=entity))
        self._Bampi_fullchannel_cache[hashable_entity] = CacheRecordFullChannel(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def get_fulluser(
        ...
        if (
            not force
            and self._Bampi_fulluser_cache.get(hashable_entity)
            and not self._Bampi_fulluser_cache[hashable_entity].expired()
            and self._Bampi_fulluser_cache[hashable_entity].ts + exp > time.time()
        ):
            return self._Bampi_fulluser_cache[hashable_entity].full_user

        result = await self(GetFullUserRequest(entity))
        self._Bampi_fulluser_cache[hashable_entity] = CacheRecordFullUser(
            hashable_entity,
            result,
            exp,
        )
    ...
    async def _call(
        ...
        # list and you will be banned from Bampi federation.
